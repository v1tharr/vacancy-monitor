...
Парсер вакансий с сайтов IT-компаний. Использует Playwright для рендеринга
...
```json
{
  ...
  "timeouts": { "page_ms": 30000, "link_ms": 20000, "inter_page_sec": 2.0 }
}
```

## Режимы запуска

### Разовый запуск (с ПК, вручную)
...
### Серверный режим (автозапуск по расписанию)
...
### Linux — systemd timer (рекомендуется)
...
```
vacancy-monitor/
├── vacancy_monitor.py      # основной скрипт             (в git)
